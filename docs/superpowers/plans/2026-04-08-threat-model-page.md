# Threat Model Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `/threat-model` — a static, long-form research page explaining 5 mechanisms by which AI-native startups outcompete traditional businesses, grounded in real company evidence and Manitoba context.

**Architecture:** Single Next.js 15 Server Component (`src/app/threat-model/page.tsx`) with all content inline — no data fetching, no shared components, no new types. Three supporting edits: nav.ts splits the current "Threat Model" entry into two (Threat Model → `/threat-model`, Threat Simulator → `/threat-simulator`), and ThreatSimulatorClient.tsx gets a cross-link footer.

**Tech Stack:** Next.js 15 App Router, TypeScript, Tailwind CSS v4, existing CSS variables from `globals.css`

**Spec:** `docs/superpowers/specs/2026-04-08-threat-model-page-design.md`
...
|------|--------|----------------|
| `src/lib/nav.ts` | Edit | Split "Threat Model" entry into two: Threat Model + Threat Simulator |
| `src/app/threat-model/page.tsx` | Create | Full static page — hero, 5 mechanism sections, Manitoba section, CTA strip |
| `src/app/threat-simulator/ThreatSimulatorClient.tsx` | Edit | Add cross-link to `/threat-model` at bottom |
...
Open `src/lib/nav.ts`. Confirm line 10 reads:
```ts
{ href: "/threat-simulator", label: "Threat Model" },
```

- [ ] **Step 2: Split into two entries**

Replace that single entry with two:
```ts
...
git commit -m "fix: split nav — Threat Model (/threat-model) + Threat Simulator (/threat-simulator)"
```

---

## Task 2: Create the threat-model page — hero + metadata

**Files:**
- Create: `src/app/threat-model/page.tsx`

Pattern reference: look at `src/app/threat-simulator/page.tsx` for hero structure (dark navy background, amber accent stripe, breadcrumb, eyebrow/H1/subheading pattern). Also check `src/app/policy/page.tsx` for prose section styling.

- [ ] **Step 1: Create the file with metadata + hero only**
...
npm run build
```
Expected: clean build. Route `/threat-model` now appears in the output. No TypeScript errors.

- [ ] **Step 3: Commit**

```bash
git add src/app/threat-model/page.tsx
git commit -m "feat: add /threat-model page — hero section"
```

---

## Task 3: Mechanism 01 — Price Floor Collapse

**Files:**
- Modify: `src/app/threat-model/page.tsx`

Each mechanism section uses this HTML structure — note it carefully, all 5 follow the same pattern:

```
<section> with alternating background (paper / paper-deep)
  <div max-w-4xl>
    [number badge + mechanism name header]
    [plain-English definition]
    [stat callout box — amber tint, gold left border]
    [evidence paragraphs]
    [Manitoba sectors tag list]
    ["What to watch for" box — neutral surface]
  </div>
</section>
```

- [ ] **Step 1: Add mechanism 01 section after the hero**

Replace the `{/* Mechanism sections ... go here */}` comment with:
...
```bash
git add src/app/threat-model/page.tsx
git commit -m "feat: threat-model — mechanism 01 price floor collapse"
```

---

## Task 4: Mechanisms 02, 03, 04, 05

**Files:**
- Modify: `src/app/threat-model/page.tsx`

Add all four remaining mechanism sections. Use **alternating backgrounds**: 02 = `var(--color-paper-deep)`, 03 = `var(--color-paper)`, 04 = `var(--color-paper-deep)`, 05 = `var(--color-paper)`. Same HTML structure as mechanism 01 — only the colour, content, and `aria-labelledby` id change.

- [ ] **Step 1: Add mechanism 02 — Speed Arbitrage**
...
- [ ] **Step 2: Add mechanism 03 — Scale Without Headcount**
...
- [ ] **Step 3: Add mechanism 04 — Junior Work at AI Quality**
...
- [ ] **Step 4: Add mechanism 05 — Always-On, No Overhead**
...
```bash
git add src/app/threat-model/page.tsx
git commit -m "feat: threat-model — mechanisms 02–05"
```

---

## Task 5: Manitoba section + CTA strip

**Files:**
- Modify: `src/app/threat-model/page.tsx`

- [ ] **Step 1: Add the Manitoba section**

Append after mechanism 05 section:
...
- [ ] **Step 2: Add the CTA strip**

Append immediately after the Manitoba section:
...
```bash
git add src/app/threat-model/page.tsx
git commit -m "feat: threat-model — Manitoba section and CTA strip"
```

---

## Task 6: Cross-link from threat-simulator

**Files:**
- Modify: `src/app/threat-simulator/ThreatSimulatorClient.tsx`

The file is 275 lines. The bottom of the component (after all the scenario content) needs a plain footer link.

- [ ] **Step 1: Read the bottom of the file**

Read `src/app/threat-simulator/ThreatSimulatorClient.tsx` lines 240–275 to find the closing JSX of `ThreatSimulatorClient`.

- [ ] **Step 2: Add the cross-link**

Before the final closing tag of the `ThreatSimulatorClient` return statement, add:
...
Confirm `Link` is already imported at the top of the file — it is (line 4: `import Link from "next/link"`).
...
```bash
git add src/app/threat-simulator/ThreatSimulatorClient.tsx
git commit -m "feat: threat-simulator — add cross-link to /threat-model research page"
```
...
npm run build
```
Expected: clean build. Confirm `/threat-model` appears in the route list. Count should be one higher than before.

- [ ] **Step 2: Check all 12 verification items from the spec**

Walk through `docs/superpowers/specs/2026-04-08-threat-model-page-design.md` Section 8 checklist:

1. ✅ Build clean, `/threat-model` in route list
2. ✅ `npx tsc --noEmit` — no errors
3. Visit `/threat-model` — confirm 5 mechanism sections each have their colour-coded number badge
4. Confirm each stat callout box has amber background + gold left border
5. Confirm each "What to watch for" box is present
6. Confirm Manitoba section shows Stats Canada stat grid and Spellbook callout
7. Confirm CTA strip has 3 cards: threat-simulator, calculator, policy
8. Visit `/threat-simulator` — confirm "Read the research behind this →" link appears at bottom
9. Scan page text — no unexplained acronyms, no jargon (LLM is acceptable if explained on first use)
10. Resize to mobile width — confirm single-column layout, no horizontal overflow
11. Open nav — confirm both "Threat Model" and "Threat Simulator" appear as separate entries
12. Click "Threat Model" in nav — confirm it goes to `/threat-model`, not `/threat-simulator`
